...
describe("dnMask", () => {
  ...
  it("preserves 4-fold symmetry at integer-aligned pixel offsets (no rounding error)", () => {
    ...
    expect(cn[toIdx(center + a, center + b)]).toBeCloseTo(cn[toIdx(center - b, center + a)], 10);
  });
});
```
...
describe("SceneRegistry", () => {
  ...
  it("duplicate register with same id but different object also throws", () => {
    ...
    expect(() => registry.register(makeMockModule("singularity"))).toThrowError(
      /already registered/,
    );
  });
});
...
export function useActiveScene({
  ...
  useEffect(() => {
    ...
    const sentinels = Array.from(document.querySelectorAll<HTMLElement>("[data-scene-id]"));
...
const sceneImportMap: Partial<Record<SceneId, React.LazyExoticComponent<React.FC>>> = {
  singularity: lazy(async () => {
    ...
    const SceneComponent: React.FC = (props) =>
      React.createElement(
        mod.singularityModule.Scene,
        props as Parameters<typeof mod.singularityModule.Scene>[0],
      );
...
export function SceneHost({ activeSceneId }: SceneHostProps): React.ReactElement {
  const LazyScene = useMemo(() => sceneImportMap[activeSceneId] ?? null, [activeSceneId]);
...
export function PresetMenu({
  ...
  return (
    <div
      ...
      style={{ display: "flex", flexDirection: "column", gap: "4px", minWidth: "200px" }}
    >
...
const defaultProps = {
  ...
  formula:
    "I = 1 - exp(-exp(c.x * 0.7) / w.x / (2 + i²/4 - i) / (0.5 + 1/a) / (0.03 + |p| - 0.7) * intensity)",
...
function SingularitySceneWithControls({
  ...
  const config = useControls("Singularity", {
    ...
    waveStrength: {
      value: initialConfig.waveStrength,
      min: 0.0,
      max: 2.0,
      step: 0.05,
      label: "Wave Strength",
    ...
    colorShift: {
      value: initialConfig.colorShift,
      min: -1.0,
      max: 1.0,
      step: 0.05,
      label: "Color Shift",
...
export function pushPosition(buf: TrailBuffer, x: number, y: number, z: number): void {
 ...
 */
export function readTrail(buf: TrailBuffer, out: Float32Array, outOffset = 0): number {
  const n = buf.count;
  if (n === 0) return 0;
...
export function LorenzScene({ config, perf: _perf, symmetry: _symmetry }: LorenzSceneProps) {
  const { sigma, rho, beta, particleCount, dt, trailLength } = useControls("Lorenz", {
...
export function magneticField(position: Vec3, sources: MagneticSource[]): Vec3 {
  return sources.reduce<Vec3>((acc, src) => add(acc, dipoleField(position, src)), {
    x: 0,
    y: 0,
    z: 0,
  });
...
export function MagneticScene({ config, perf: _perf, symmetry: _symmetry }: MagneticSceneProps) {
  const { particleCount, charge, mass, B0, dt, symmetryOrder } = useControls("Magnetic", {
    /* config merged key-by-key — see Scene.tsx */
  });
...
export const GRAY_SCOTT_LEVA_SCHEMA = {
  F: { value: 0.03, min: 0.005, max: 0.1, step: 0.001 },
...
function buildColormapTexture(name: "viridis" | "magma" | "grayscale"): THREE.DataTexture {
  ...
  for (let i = 0; i < size; i++) {
    ...
    if (name === "grayscale") {
      ...
      g = Math.round((0.0 + t * 0.293 + t * t * 0.707) * 255);
 ...
 */
function nonlinearTerm(u_hat: Float64Array, k: Float64Array, N: number): Float64Array {
...
export function ksStep(state: KSState, config: KSConfig): KSState {
  ...
  const u_hat_next = etdrk4Step(u_hat, L_hat, (uh) => nonlinearTerm(uh, k, N), dt);
...
export function KuramotoSivashinskyScene() {
  ...
  const spaceTimeTex = useMemo(() => {
    ...
    const tex = new THREE.DataTexture(data, N, SPACETIME_ROWS, THREE.RedFormat, THREE.FloatType);
    tex.needsUpdate = true;
    return tex;
  }, [N]);
...
export default function Home() {
  ...
  return (
    <section id="home" className="relative min-h-screen flex items-center">
      ...
      */}
      <div data-scene-id="singularity" aria-hidden="true" className="absolute inset-0 -z-10" />

      {/* Hero copy — unchanged from pre-migration */}
      <div className="relative z-10 hero-content">{/* existing hero JSX preserved here */}</div>
    </section>
  );
}
```
...
const SIM_NAMES = (
  process.env.SIM_NAMES ?? "singularity,lorenz,magnetic,grayScott,kuramotoSivashinsky"
).split(",");
...
async function runWithPlaywright() {
  ...
  for (const name of SIM_NAMES) {
    ...
    console.log(
      `[perf] ${name}: p95 = ${p95.toFixed(2)}ms  budget = ${budget}ms  ${pass ? "PASS" : "FAIL"}`,
    );
  }

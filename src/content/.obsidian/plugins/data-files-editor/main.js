    ...
    if ((t && typeof t == "object") || typeof t == "function")
      for (let O of ze(t))
        !je.call(e, O) &&
          ...
          W(e, O, {
            get: () => t[O],
            enumerable: !(o = Ge(t, O)) || o.enumerable,
          });
    return e;
  };
var Be = (e, t, s) => (
...
var Ke = f.LRLanguage.define({
  name: "json",
  parser: fe.configure({
    props: [
      f.indentNodeProp.add({
        Object: (0, f.continuedIndent)({ except: /^\s*\}/ }),
        Array: (0, f.continuedIndent)({ except: /^\s*\]/ }),
      }),
      f.foldNodeProp.add({ "Object Array": f.foldInside }),
    ],
  }),
  languageData: {
...
function me() {
  return new f.LanguageSupport(Ke);
}
...
var g = require("@codemirror/view"),
  ...
  Te = (() => [
    ...
    (0, g.drawSelection)(),
    (0, g.dropCursor)(),
    Qe.EditorState.allowMultipleSelections.of(!0),
    (0, m.indentOnInput)(),
...
var h = class extends Se.TextFileView {
  ...
  setViewData(s, o) {
    this.cmEditor.dispatch({
      changes: { from: 0, to: this.cmEditor.state.doc.length, insert: s },
    });
  }
  clear() {}
  getDisplayText() {
  ...
  Q = class {
    constructor(t, s, o) {
      ((this.parent = t),
        (this.depth = s),
        (this.type = o),
        (this.hash = (t ? (t.hash + t.hash) << 8 : 0) + s + (s << 4) + o));
    }
  };
Q.top = new Q(null, -1, Qt);
function V(e, t) {
  for (let s = 0, o = t - e.pos - 1; ; o--, s++) {
    let O = e.peek(o);
    if (u(O) || O == -1) return s;
  }
}
...
var ut = new T.ContextTracker({
  start: Q.top,
  reduce(e, t) {
    return e.type == _ && (t == pt || t == Pt) ? e.parent : e;
  },
  shift(e, t, s, o) {
    if (t == be) return new Q(e, V(o, o.pos), Oe);
    if (t == ve || t == ye) return new Q(e, V(o, o.pos), oe);
    if (t == L) return e.parent;
    if (t == ct || t == mt) return new Q(e, 0, _);
    if (t == ke && e.type == ne) return e.parent;
    if (t == ht) {
      let O = /[1-9]/.exec(o.read(o.pos, s.pos));
...
var Tt = new T.ExternalTokenizer(
  ...
  St = new T.ExternalTokenizer(
    (e, t) => {
      if (t.context.type == _) {
        e.next == 63 && (e.advance(), E(e.next) && e.acceptToken(nt));
        return;
      }
      if (e.next == 45)
        (e.advance(),
...
function Ee(e, t) {
  return e.next == 37
    ? (e.advance(), Ce(e.next) && e.advance(), Ce(e.next) && e.advance(), !0)
...
function re(e) {
  for (e.advance(); !E(e.next) && M(e.next) != "f"; ) e.advance();
}
function le(e, t) {
  let s = e.next,
    o = !1,
  ...
  for (e.advance(); ; ) {
    ...
    else if (i == 92 && s == 34) e.next >= 0 && e.advance();
    else if (u(i)) {
      if (t) return !1;
      o = !0;
...
function Ve(e, t, s, o) {
  ...
  for (;;) {
    ...
    if (
      ...
      break;
    if (t && M(i) == "f") return !1;
    for (let n = d; n >= 0; n--) e.advance();
    if (t && e.pos > O + 1024) return !1;
  }
  return !0;
}
var Ct = new T.ExternalTokenizer((e, t) => {
    if (e.next == 33) (we(e), e.acceptToken(gt));
      ...
      e.next == 39 || e.next == 34
        ...
        : Ve(e, !1, t.context.type == _, t.context.depth) && e.acceptToken(rt);
  }),
  Et = new T.ExternalTokenizer((e, t) => {
    ...
    e: for (;;) {
      ...
      if (
        (!O && (w(e, 45, O) || w(e, 46, O))) ||
        (!u(i) && (s < 0 && (s = Math.max(t.context.depth + 1, O)), O < s))
      )
        break;
      for (;;) {
        if (e.next < 0) break e;
        let d = u(e.next);
        if ((e.advance(), d)) continue e;
        o = e.pos;
      }
    }
    e.acceptTokenTo(ke, o);
  }),
  bt = (0, l.styleTags)({
    DirectiveName: l.tags.keyword,
    DirectiveContent: l.tags.attributeValue,
    "DirectiveEnd DocEnd": l.tags.meta,
    QuotedLiteral: l.tags.string,
    BlockLiteralHeader: l.tags.special(l.tags.string),
    BlockLiteralContent: l.tags.content,
    Literal: l.tags.content,
    "Key/Literal Key/QuotedLiteral": l.tags.definition(l.tags.propertyName),
    "Anchor Alias": l.tags.labelName,
    Tag: l.tags.typeName,
    Comment: l.tags.lineComment,
    ": , -": l.tags.separator,
    "?": l.tags.punctuation,
    "[ ]": l.tags.squareBracket,
    "{ }": l.tags.brace,
  }),
  _e = T.LRParser.deserialize({
    version: 14,
    states:
    ...
    tokenizers: [Tt, St, Ct, Et, 0, 1],
    topRules: { Stream: [0, 15] },
    tokenPrec: 0,
  });
var p = require("@codemirror/language"),
  yt = require("@lezer/common"),
  U = require("@lezer/highlight"),
  Ye = require("@lezer/lr"),
  vt = Ye.LRParser.deserialize({
    version: 14,
    states:
    ...
    parser: _e.configure({
      props: [
        p.indentNodeProp.add({
          ...
          FlowMapping: (0, p.delimitedIndent)({ closing: "}" }),
          FlowSequence: (0, p.delimitedIndent)({ closing: "]" }),
        }),
        p.foldNodeProp.add({
          "FlowMapping FlowSequence": p.foldInside,
...
function We() {
  return new p.LanguageSupport(kt);
}
var os = p.LRLanguage.define({
  name: "yaml-frontmatter",
  parser: vt.configure({
    props: [(0, U.styleTags)({ DashLine: U.tags.meta })],
  }),
});
var Z = require("@codemirror/language"),
  b = require("@lezer/highlight");
var Lt = Z.HighlightStyle.define([
    { tag: b.tags.propertyName, color: "#e5c07b" },
    { tag: b.tags.string, color: "#98c379" },
    { tag: b.tags.number, color: "#d19a66" },
    { tag: b.tags.bool, color: "#d19a66" },
    { tag: b.tags.null, color: "#d19a66" },
    { tag: b.tags.comment, color: "#7f848e" },
  ]),
  y = class extends h {
    constructor(t, s) {
      super(t, s);
...
function $e(e) {
  return [
    ...e.workspace.getLeavesOfType(X).filter((s) => s.view instanceof R),
    ...e.workspace.getLeavesOfType(c).filter((s) => s.view instanceof C),
    ...e.workspace.getLeavesOfType(x).filter((s) => s.view instanceof y),
  ].map((s) => s.view);
}
var N = require("obsidian"),
  De = Be(require("obsidian"));
function de(e, t, s) {
  if ((0, N.requireApiVersion)("1.11.0")) {
    let o = De.SettingGroup;
    if (o) {
      let O = new o(e);
      return (
        t && O.setHeading(t),
...
var Y = class extends qe.PluginSettingTab {
  ...
  display() {
    ...
    (d.addSetting((a) =>
      ...
      d.addSetting((a) =>
        a.setName("Wrap long lines").addToggle((r) =>
          r.setValue(this.plugin.settings.lineWrapping).onChange(async (n) => {
            ((this.plugin.settings.lineWrapping = n),
...
var G = class extends z.Plugin {
  async onload() {
    (await this.loadSettings(),
      this.registerView(c, (s) => new C(s, this)),
      this.registerView(X, (s) => new R(s, this)),
      this.registerView(x, (s) => new y(s, this)),
      this.TryRegisterTxt(),
  ...
  onunload() {}
  async loadSettings() {
    this.settings = Object.assign({}, Ze, await this.loadData());
  }
  async saveSettings() {
    await this.saveData(this.settings);
  }
  registerContextMenuCommand(s, o) {
    this.registerEvent(
      this.app.workspace.on("file-menu", (O, i) => {
        let d = i instanceof z.TFile ? i.parent : i;
        O.addItem((a) => {
          a.setTitle(`New .${s} file`)
            .setIcon(o)
            .setSection("action")
            .onClick(async () => {
  ...
  async createFile(s, o) {
    let { vault: O } = this.app,
      ...
      a = Ue.join(s, `${d}.${o}`);
    try {
      if (await i.exists(a)) throw new Error(`${a} already exists`);
      let n = await O.create(a, "");
      await this.app.workspace.getLeaf(!0).openFile(n);
    } catch (r) {

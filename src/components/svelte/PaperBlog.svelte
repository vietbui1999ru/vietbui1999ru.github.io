<script lang="ts">
  ...
  // Journal-style date stamp: 2026·05·24
...
<section id="blog" class="relative w-full py-24">
    ...
    <ul class="border-t border-journal-3">
      {#each posts as post (post.slug)}
        <li class="border-b border-journal-3">
          <a
            href="/blog/{post.slug}"
            class="group flex items-baseline gap-5 px-1 py-4 no-underline transition-all duration-200 hover:bg-journal-4 hover:pl-4"
          >
            <span class="w-24 shrink-0 font-mono text-step--1 text-journal-2"
              >{stamp(post.date)}</span
            >
            <span class="flex-1 font-serif text-step-1 font-medium text-ink">{post.title}</span>
            <span
              class="font-mono text-ink opacity-0 transition-opacity duration-200 group-hover:opacity-100"
              >→</span
            >
          </a>
        </li>
